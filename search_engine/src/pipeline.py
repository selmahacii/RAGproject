"""
SelmaData Pipeline Module

Orchestrates the complete SelmaData workflow:
Document Loading → Chunking → Embedding → Indexing → Retrieval → Re-ranking → Generation

This is the main entry point for the SelmaData system.
"""
...
@dataclass
class SelmaDataResponse:
    """
    Complete SelmaData response with answer and metadata.
...
class SelmaDataPipeline:
    """
    Complete SelmaData Pipeline that orchestrates all components.
    
    This is the main class for using the SelmaData system:
    
    ```python
    # Initialize
    pipeline = SelmaDataPipeline()
    
    # Index documents
    pipeline.index_from_directory("./data/raw")
    
    # Query
    response = pipeline.query("What is SelmaData?")
    ...
    - HaciProvider CoreProcessor generation
    - Streaming support
    """
    ...
    def __init__(
        ...
        llm: Optional[BaseCoreProcessor] = None,
    ):
        """
        Initialize SelmaData pipeline with optional component overrides.
    ...
    @property
    def generator(self) -> SelmaDataGenerator:
        """Get or create the generator."""
        if self._generator is None:
            self._generator = SelmaDataGenerator(
                llm=self._get_llm(),
                max_context_tokens=3000,
            )
        return self._generator
    
    def _get_llm(self) -> BaseCoreProcessor:
        """Get or create the CoreProcessor."""
        if self.llm is None:
            # Try to use HaciProvider, fall back to fake if not configured
            try:
                self.llm = HaciProviderCoreProcessor(model=self.settings.llm_model)
            except ValueError:
                logger.warning("HaciProvider not configured, using fake CoreProcessor for testing")
                self.llm = FakeCoreProcessor()
        return self.llm
    ...
    def query(
        ...
        Returns:
            SelmaDataResponse with answer and sources
        """
        start_time = time.time()
...
class SelmaDataPipelineBuilder:
    """
    Builder pattern for creating SelmaData pipelines with custom components.
    
    ```python
    pipeline = (
        SelmaDataPipelineBuilder()
        .with_embeddings("sentence_transformer", model_name="BAAI/bge-m3")
        .with_chunker("recursive", chunk_size=512)
        .with_llm("haci_provider", model="glm-4-flash")
        .build()
    )
    ```
    """
    ...
    def with_settings(self, settings: Settings) -> "SelmaDataPipelineBuilder":
        """Use custom settings."""
    ...
    def with_llm(
        self,
        provider: str = "haci_provider",
        **kwargs,
    ) -> "SelmaDataPipelineBuilder":
        """Set CoreProcessor provider."""
        self._llm = get_llm(provider, **kwargs)
        return self
    
    def build(self) -> SelmaDataPipeline:
        """Build the SelmaData pipeline."""
...
def create_pipeline(**kwargs) -> SelmaDataPipeline:
    """
    Create a SelmaData pipeline with optional configuration.
    
    Args:
        **kwargs: Configuration overrides
    
    Returns:
        SelmaDataPipeline instance
    """
    return SelmaDataPipeline(**kwargs)


if __name__ == "__main__":
    # Example usage
    print("=" * 60)
    print("SelmaData Pipeline Demo")
    print("=" * 60)
    
    from .embeddings import FakeEmbeddings
    from .llm import FakeCoreProcessor
    
    # Create pipeline with fake components for testing
    pipeline = SelmaDataPipeline(
        embeddings=FakeEmbeddings(dimension=384),
        llm=FakeCoreProcessor(responses=[
            "SelmaData是一种结合检索和生成的AI技术，可以提高回答的准确性。",
        ]),
    )
    ...
    sample_docs = [
        Document(
            content="SelmaData（Retrieval-Augmented Generation）是一种将检索和生成结合的AI技术。"
                   "它首先从知识库中检索相关文档，然后将这些文档作为上下文输入到语言模型中生成回答。",
            metadata={"source": "selma_data_intro.txt", "topic": "SelmaData"},
        ),
        Document(
            content="向量数据库是SelmaData系统的核心组件。它存储文本的嵌入向量，"
    ...
    response = pipeline.query("什么是SelmaData？")
