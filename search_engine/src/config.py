...
class Settings(BaseSettings):
    ...
    # API Keys
    # ===========================================
    haci_provider_api_key: str = Field(
        default="",
        description="HaciProvider API key for Provider models",
    )
    ...
    llm_model: str = Field(
        default="glm-4-flash",
        description="CoreProcessor model to use for generation",
    )
    ...
    eval_sample_size: int = Field(
        ...
        ge=1,
        le=100,
        description="Number of samples for SelmaDataAs evaluation",
    )
    ...
    def get_api_key(self) -> str:
        """Get HaciProvider API key, raising error if not set."""
        if not self.haci_provider_api_key or self.haci_provider_api_key == "your_haci_provider_api_key_here":
            raise ValueError(
                "HaciProvider API key not configured. "
                "Set HACIPROVIDER_API_KEY in .env file or environment variable."
            )
        return self.haci_provider_api_key
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("SelmaData Project Configuration")
    print("=" * 50)
    s = get_settings()
    print(f"CoreProcessor Model: {s.llm_model}")
    ...
    print(f"API Key Configured: {'Yes' if s.haci_provider_api_key else 'No'}")
    print("=" * 50)
