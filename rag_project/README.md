...
[![CI/CD](https://github.com/yourusername/rag-pipeline/actions/workflows/deploy.yml/badge.svg)](https://github.com/yourusername/rag-pipeline/actions/workflows/deploy.yml)
...
[![RAGAs](https://img.shields.io/badge/RAGAs-0.1.21-purple?style=flat-square)](https://docs.ragas.io/)

**A complete, production-ready RAG system with documented evaluation metrics**
...
**Deployed App**: [https://rag-pipeline-demo.onrender.com](https://rag-pipeline-demo.onrender.com)
...
> 📈 **Best Practice**: Recursive chunking (256) + FlashRank re-ranking achieves **0.87 average score**, exceeding all targets.
...
│                                    │                                          │
│                           ┌────────▼────────┐                                 │
│                           │   GENERATION    │  ← ZhipuAI GLM-4-flash          │
│                           │    (llm.py)     │    temperature=0.1 (factual)    │
│                           └────────┬────────┘    max_tokens=1024             │
│                                    │                                          │
│                           ┌────────▼────────┐                                 │
...
This project demonstrates a **production-grade RAG (Retrieval-Augmented Generation) pipeline** with documented evaluation metrics and real-world performance data.

### What Makes This Project Portfolio-Ready?

| Feature | Basic RAG | This Project | Measured Result |
|---------|-----------|--------------|-----------------|
...
| **Embeddings** | OpenAI API ($) | Local bge-m3 (FREE) | Cost: $0/query |
| **Evaluation** | Manual testing | RAGAs metrics | Avg Score: **0.87** |
...
### 🔍 Advanced Retrieval
- **ChromaDB**: Persistent vector storage with HNSW index
...
- **Python API**: `run_rag_pipeline()` one-liner
...
# Or use Python API
python main.py "What is RAG?" --data ./data/raw
```
...
# Run the container
docker run -p 8501:8501 -e ZHIPUAI_API_KEY=your_key rag-pipeline
...
4. Set environment: `ZHIPUAI_API_KEY`
5. Deploy!
...
│   ├── 🤖 llm.py            # GLM-4 generation + streaming
...
```python
from src.eval import build_test_dataset, evaluate_with_ragas, generate_report
...
| Issue | Solution |
|-------|----------|
| `ZHIPUAI_API_KEY not set` | Add key to `.env` file |
| `FlashRank import error` | `pip install flashrank==0.2.9` |
| `RAGAs API error` | Use `ragas==0.1.21` (pinned version) |
...
- [ZhipuAI](https://open.bigmodel.cn/) - GLM-4 LLM
...
[⬆ Back to Top](#-rag-pipeline---production-grade-retrieval-augmented-generation)
